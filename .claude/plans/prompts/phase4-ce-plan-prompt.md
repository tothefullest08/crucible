...
1. `/Users/ethan/Desktop/personal/harness/.claude/plans/03-design/final-spec.md` — **v2 최종 스펙** (Phase 1+2+3 + review 반영, 단일 진실 소스)
2. `/Users/ethan/Desktop/personal/harness/.claude/plans/03-design/final-spec-review.md` — 7-페르소나 review 결과 (P0 9건·P1 10건, 일부 v2에 반영, 잔여는 §11에 등재)
3. `/Users/ethan/Desktop/personal/harness/.claude/plans/02-research/synthesis.md` — 포팅 Top-32 자산 로드맵
...
1. **메인 태스크 분해 문서**: `.claude/plans/04-planning/implementation-plan.md`
   - 주차별 섹션
   - 각 주차 내부 태스크는 Markdown 체크리스트
   - 태스크 ID 체계: `T-W{주차}-{순번}` (예: `T-W4-03`)
   - 의존성 그래프 (Mermaid 또는 ASCII)

2. **포팅 매트릭스 문서**: `.claude/plans/04-planning/porting-matrix.md`
...
3. **§11 승격 체크리스트**: `.claude/plans/04-planning/section11-promotion-tracker.md`
   - §11-1 ~ §11-7 각 항목에 대해
   - 승격 deadline (주차)
   - 책임자 (본인 / 유저 판단 필요)
