...
- **선행 문서**:
  ...
  - `.claude/plans/2026-04-19/00-recommendations/tool-recommendations.md` — 도구 추천
  - `.claude/plans/2026-04-19/01-requirements/clarified-spec.md` — Phase 1 (10개 결정)
  - `.claude/plans/2026-04-19/02-research/*.md` (6개) + `02-research/synthesis.md` — Phase 2
  - `.claude/plans/2026-04-19/03-design/final-spec-review.md` — 7-페르소나 document-review (30 findings)
  - `.claude/plans/2026-04-19/03-design/user-decisions-5.md` — 유저 판단 5건 결정 (v3 승격 소스)
...
### 3.5 6축 강제 적용 범위 (신설, 2026-04-19 · §11-5 승격 · T-W8-PRE-01 보강 2026-04-20)

*결정 근거: user-decisions-5 §3 · §2.3 #17. 상세는 tracker §5.*

#### 3.5.1 축별 활성 규칙 (요약 — T-W8-PRE-01 신설)

진입점 분류 3단계. 세부 축별 매트릭스는 §3.5.2 참조.

| 진입점 분류 | 구성원 | 6축 검증 | 훅 강제 | 비고 |
|-----------|-------|---------|--------|------|
| **강제 검증** | `/plan` · `/verify` · `/orchestrate` | **ON** | `validate_prompt` **강제** | 축 미통과 시 산출물 배출 차단 |
| **자연 대화** | `/brainstorm` · `/compound` | 축 번호만 로그 | **강제 X** | 제안/힌트 형태, 대화 흐름 유지 |
| **OFF** | 일반 Q&A · 도구 호출 | — | — | 6축 미적용 |
...
#### 3.5.3 이스케이프 해치 `--skip-axis N`

- 문법: `/plan [요구사항] --skip-axis 2,3` (쉼표 분리, N=1~6)
- 기본 스킵은 stderr 1회 경고 + 응답 상단 `⚠️ axis N skipped` 배너
- **검증 축(5) 스킵은 강경 경고 + 추가 플래그 필수**: `--skip-axis 5 --acknowledge-risk` 조합이 없으면 skip 거부
  - stderr 강경 경고 예시: `⚠️ Axis 5 (Verify) skipped — release blocker for production deployments. Artifact will ship unverified; qa-judge · Ralph Loop are disabled.`
  - `--acknowledge-risk` 없이 `--skip-axis 5`만 지정하면 exit code `2`로 즉시 거부
- 스킵 사유는 거부 이력과 함께 `.claude/memory/corrections/skip-log.md`에 추기 (W5 `/compound` 메모리 경로 재사용)

#### 3.5.4 실효성 지표 KU (KU-6) — 2차 릴리스 유지

6축 통과가 산출물 품질 향상과 상관되는지 측정하는 KU는 **2차 릴리스로 연기 확정** (T-W8-PRE-01 재확인). MVP 기간 dogfooding 로그를 2차 KU 설계 입력으로 수집. §10.3 참조.
...
**→ §3.5 참조. v3에서 §3.5로 1차 승격 + v3.4에서 §3.5.1~§3.5.4 재구성(T-W8-PRE-01, 2026-04-20)으로 승격 완료. 실효성 KU(KU-6)는 §10.3 2차 릴리스 등재. 본 항목 미결 해제.**

### 11-6. 라이선스 · 상류 sync (P1-10)
**→ §4.5 + `porting-matrix.md` §2·§5 참조. v3에서 §4.5로 1차 승격(시나리오 A · SPDX `MIT` · DCO · 분기/반기/연 차등) + v3.4에서 상류 커밋 해시 6종 스냅샷·`NOTICES.md`·`LICENSE`·`CONTRIBUTING.md` 일괄 확정(T-W8-PRE-02, 2026-04-20)으로 승격 완료. 본 항목 미결 해제.**

### 11-7. 기타 설계 미결 → **승격 완료 (T-W8-PRE-03, 2026-04-20)**

아래 4항목은 v3.4에서 본 섹션(§11-7) 내부 본문으로 승격. 체크리스트 전부 GREEN.

#### 11-7.1 프론트매터 필드 일관성 (5 스킬 × 6 필드)

| 스킬 | name | description | when_to_use | input | output | validate_prompt |
|-----|------|-------------|------------|------|--------|-----------------|
| `brainstorm` | ✅ | ✅ (한·영) | ✅ | ✅ | ✅ | ✅ (Plan 3축 자기검증) |
| `plan` | ✅ | ✅ (한·영) | ✅ | ✅ | ✅ | ✅ (Plan 3축 자기검증, T-W8-05에서 추가) |
| `verify` | ✅ | ✅ (한·영) | ✅ | ✅ | ✅ | ✅ (검증 축) |
| `compound` | ✅ | ✅ (한·영) | ✅ | ✅ | ✅ | ✅ (개선 축 · 6 compound) |
| `orchestrate` | ✅ | ✅ (한·영) | ✅ | ✅ | ✅ | ✅ (4축 순차 무결성) |

필드 6종 × 스킬 5종 = 30 셀 전부 ✅. `plan`의 `validate_prompt` 보강은 T-W8-05에서 실행.

#### 11-7.2 포지셔닝 1문장 (README 최상단 전용, ≤140자)

§1 TL;DR 2문장 버전은 세부 비교 포함. README 최상단 배지 위치용 단축안 확정:

- **영문 (≤140)**: `harness compounds only user-approved learnings into durable memory across a six-axis Brainstorm→Plan→Verify→Compound Claude Code loop.`
- **한국어 (≤140)**: `harness는 승격 게이트 통과 학습만 저장하는, 6축(Brainstorm→Plan→Verify→Compound) 컴파운딩 메모리 Claude Code 플러그인입니다.`

영문 136자 · 한국어 88자 (공백 포함). README.md · README.ko.md 최상단 인용 블록에 동일 배치.

#### 11-7.3 OSS composability (스킬 독립 사용 + 의존 DAG)

각 스킬의 **독립 사용 가능성**과 의존 관계:

| 스킬 | 단독 사용 | 필수 선행 | 선택적 후속 | 비고 |
|-----|----------|---------|-----------|------|
| `/brainstorm` | ✅ 가능 | — | `/plan` | requirements.md만 배출, 후속 없이도 사용 |
| `/plan` | ✅ 가능 | requirements.md (경로 입력) | `/verify`·`/compound` | `/brainstorm` 없이 수동 requirements로 가능 |
| `/verify` | ✅ 가능 | 산출물 경로 | `/compound` | 어떤 산출물에도 적용 (자체 qa-judge) |
| `/compound` | ✅ 가능 | — (3 트리거) | — | 세션 독립, 메모리 저장만 수행 |
| `/orchestrate` [Stretch] | △ 조건부 | 위 4 스킬 전부 | — | 4 스킬 오케스트레이션, 개별 스킬 미탑재 시 grace fallback |

**의존 DAG**:

```
  /brainstorm ──► /plan ──► /verify ──► /compound
      │             │          │           ▲
      └─────────────┴──────────┴───────────┤  (수동 /compound 호출 가능)
                                           │
  /orchestrate ──► 위 4 스킬 순차 호출 (composition)
```

각 스킬은 `skills/{name}/SKILL.md` 단일 디렉토리 복사만으로 독립 등록 가능. `/orchestrate`는 4 스킬 composition이므로 나머지 4종과 함께 배포 필수.

#### 11-7.4 `/orchestrate` 실질 가치 비교표 (수동 4회 vs 1회)

| 항목 | 수동 호출 (4회) | `/orchestrate` (1회) |
|-----|---------------|---------------------|
| 타이핑 횟수 | 4 프롬프트 + 3 경로 매개변수 수동 전달 | 1 프롬프트 (주제 1줄) |
| 컨텍스트 보존 | 각 호출 간 유저가 경로/상태 직접 인계 | experiment-log.yaml CP-0~CP-5 자동 전달 |
| 체크포인트 | 없음 (각 호출 독립 종료) | CP-0~CP-5 디스크 영속 (중단·재개 지원) |
| 실패 복구 | 수동 재시작 + 이전 산출물 재지정 | 해당 축에서 중단 + 이미 진행된 체크포인트 보존, 재개 시 잔여 축만 수행 |

수동 4회는 유연성 우위, `/orchestrate`는 반복 워크플로·장기 실행·컨텍스트 전달 자동화 우위. MVP는 양쪽 다 제공 (수동 4 스킬은 항상 가용).
