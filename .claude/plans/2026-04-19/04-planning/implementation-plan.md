...
> **입력**: `.claude/plans/2026-04-19/03-design/final-spec.md` v2 (단일 진실 소스)
...
- [x] **T-W0-05** W0 게이트 판정 → W1 착수 / 재스코프 / §11 재설계
  - 의존성: T-W0-01 ~ T-W0-04
  - 공수: 1h
  - 검증: `.claude/plans/2026-04-19/04-planning/w0-gate-decision.md` 작성 (TL;DR 유지 여부 판정)
...
- [x] **T-W7.5-PRE-01** ⛳ §11-4 승격 — KU 실행 상세 정식화
...
- [x] **T-W7.5-01** KU-0 qa-judge 점수 histogram 수집 + 분위수 기반 임계값 재조정 → **AC-7**
  - 의존성: T-W7.5-PRE-01
  - 공수: 4h
  - 검증: histogram 플롯 + 임계값 0.80/0.40 → 분위수 기반 값으로 갱신 diff

- [x] **T-W7.5-02** KU-1 validate_prompt 훅 발동률 ≥ 99% + 응답률 ≥ 90% 측정 → **AC-3**
  - 의존성: T-W7.5-PRE-01
  - 공수: 4h
  - 검증: 10 샘플 태스크 실측 + judge 에이전트 평가. 미달 시 T-W4-05 재작업 게이트

- [x] **T-W7.5-03** KU-2 description 한·영 병기 A/B 20 발화 정확도 → **AC-4**
  - 의존성: T-W7.5-PRE-01
  - 공수: 4h
  - 검증: 영어만 vs 한·영 병기 정확도 차 ≤ 5%p (양방향 기준 — P1-3)

- [x] **T-W7.5-04** KU-3 승격 게이트 false positive < 20% 측정 → **AC-5**
  - 의존성: T-W7.5-PRE-01
  - 공수: 4h
  - 검증: 10 실 세션 기반 오검지율 20% 미만

- [x] **T-W7.5-05** 하드닝: W4-02 verify 에이전트 잔여 구현 이월
  - 의존성: T-W4-02
  - 공수: 4h
  - 검증: 6 에이전트 실구현 완결 + 스텁 제거

- [x] **T-W7.5-06** 하드닝: W6 oscillation 과적합 방지 구현 이월
  - 의존성: T-W6-05
